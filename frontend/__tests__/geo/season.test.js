...
test('first sight in a new season carries last season in and pays its reward once; the board is per season', async () => {
  ...
  // Ada led the placed pool: Sapphire. Grace played two games: nothing.
  expect((await store.getProfileById(ada.id)).points).toBe(SEASON_REWARDS.Sapphire);
  expect((await store.getProfileById(grace.id)).points).toBe(0);
  const ledger = store._dump().ledger;
  expect(ledger).toHaveLength(1);
  expect(ledger[0]).toMatchObject({ profileId: ada.id, amount: 350, ref: 'season:s0:classic', reason: 'Season 0 classic: Sapphire' });
