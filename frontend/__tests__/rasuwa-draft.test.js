...
describe('snapshot and restore round trip', () => {
  ...
  test('hand edits from an older letter template restore rebuilt, and say so', () => {
    ...
    // v2 predates the August 31 facts; its hand edits rebuild too.
    ...
    // v4 predates the named ministries and list counts; same rebuild.
    ...
    // v6 predates the emergency-disclosure ask; same rebuild.
    const v6 = { ...snapshotDraft(state), templateVersion: 6 };
    expect(restoreDraft(v6).overrides).toEqual({});
    expect(restoreDraft(v6).templateOutdated).toBe(true);
    // v7 predates the Foreign Minister's CNN interview facts; same rebuild.
