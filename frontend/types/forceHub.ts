 ...
 * Backend should return data matching ForceHubData from GET /api/forces/[citySlug]/hub
 */
...
export type HubChatMessage = {
  ...
  content: string;
  createdAt: string;        // ISO timestamp
  divisionId?: string;      // null = force-wide
...
export type HubRequest = {
  ...
  title: string;
  body: string;
  divisionId: string | null; // null = force-wide
...
export type HubMember = {
  ...
  name: string;
  avatarUrl?: string;
  role: 'MEMBER' | 'DIVISION_LEAD' | 'FORCE_LEAD' | 'ADMIN';
  divisionId?: string;
  divisionName?: string;
  isOnDuty: boolean;
  lastActiveAt?: string;
...
export type SquadMembership = {
  ...
  divisionIds: string[];    // divisions they belong to in this force
  role: 'MEMBER' | 'DIVISION_LEAD' | 'FORCE_LEAD' | 'ADMIN' | null;
};
...
export type ForceHubContextValue = ForceHubState & ForceHubActions;
