...
/**
 * Force Settings Page - Updated with ReunitePets Design System
...
export default function ForceSettingsPage() {
  ...
  const [force, setSquad] = useState(null);
  ...
  const fetchSquadData = useCallback(async () => {
    ...
    try {
      ...
      setSquad(data.force);
      setMembers(data.force.members || []);
      setDivisions(data.force.divisions || []);

      // Find user's role
      const userMembership = data.force.members?.find(m => m.userId === session.user.id);
      ...
      // Initialize settings
      setSettings({
        description: data.force.description || '',
        specializesInDogs: data.force.specializesInDogs,
        specializesInCats: data.force.specializesInCats,
        specializesInBirds: data.force.specializesInBirds,
        specializesInOther: data.force.specializesInOther,
        availableWeekdays: data.force.availableWeekdays,
        availableWeekends: data.force.availableWeekends,
        availableDay: data.force.availableDay,
        availableNight: data.force.availableNight,
        hasTrackingDogs: data.force.hasTrackingDogs,
        hasDrones: data.force.hasDrones,
        isAcceptingCases: data.force.isAcceptingCases,
      });
    } catch (err) {
      console.error('Error fetching force:', err);
      setError('Failed to load force settings');
    } finally {
      setLoading(false);
    }
  }, [forceId, session?.user?.id]);

  useEffect(() => {
    if (sessionStatus === 'unauthenticated') {
      router.push(`/login?callbackUrl=/rescue-forces/${forceId}/settings`);
      return;
    }
  ...
  }, [sessionStatus, fetchSquadData, router, forceId]);
  ...
  const handleUpdateMemberRole = async (memberId, newRole) => {
    ...
    try {
      const res = await fetch(`/api/rescue-forces/${forceId}/members/${memberId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-midnight-100 to-midnight-200 p-4">
        <Card className="max-w-md mx-auto mt-16 text-center p-8">
          ...
          <Link href={`/rescue-forces/${forceId}`}>
    ...
    <div className="min-h-screen bg-gradient-to-b from-midnight-100 to-midnight-200">
      ...
      <div className="bg-white border-b border-midnight-200 px-4 py-6">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-midnight-500 mt-1">{force?.name}</p>
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl hover:bg-midnight-50 transition">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-midnight-500 mt-0.5">Turn off when force is at capacity</p>
                  </div>
                </label>
              </div>
            </Card>
