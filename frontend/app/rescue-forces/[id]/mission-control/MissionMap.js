...
export default function MissionMap({ force, cases, selectedMission, onSelectCase }) {
  ...
  useEffect(() => {
    ...
    // Default to force center or Chicago
    const center = force?.centerLatitude && force?.centerLongitude
    ...
    if (force?.centerLatitude && force?.centerLongitude) {
      ...
      L.marker([force.centerLatitude, force.centerLongitude], { icon: squadIcon })
        .addTo(mapInstanceRef.current)
        .bindPopup(`<b>${force.name}</b><br>Force Headquarters`);

      // Add radius circle
      if (force.radiusMiles) {
        L.circle([force.centerLatitude, force.centerLongitude], {
          radius: force.radiusMiles * 1609.34, // Convert miles to meters
          color: '#3b82f6',
          fillColor: '#3b82f6',
          fillOpacity: 0.05,
          weight: 1,
