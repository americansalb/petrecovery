...
/**
 * Division Management Page - Updated with ReunitePets Design System
 * Uses: Midnight Blue + Flashlight Yellow color palette
 *
 * Allows force founders/leaders to:
...
export default function DivisionsManagementPage() {
  ...
  const loadSquadAndDivisions = async () => {
    try {
      // Load force details
      const forceRes = await fetch(`/api/rescue-forces/${forceId}`);
      const forceData = await forceRes.json();

      if (!forceRes.ok) {
        throw new Error(forceData.error || 'Failed to load force');
      }

      setSquad(forceData.force);

      // Check user role
      if (session?.user?.id && forceData.force.members) {
        const membership = forceData.force.members.find(
      ...
      // Load divisions
      const divRes = await fetch(`/api/rescue-forces/${forceId}/divisions`);
      if (divRes.ok) {
  ...
  const handleUpdateDivision = async (e) => {
    ...
    try {
      const res = await fetch(`/api/rescue-forces/${forceId}/divisions/${editingDivision.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteDivision = async (divisionId) => {
    ...
    try {
      const res = await fetch(`/api/rescue-forces/${forceId}/divisions/${divisionId}`, {
        method: 'DELETE',
      });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-midnight-100 to-midnight-200 p-4">
        <Card className="max-w-md mx-auto mt-16 text-center p-8">
          ...
          <p className="text-midnight-500 mb-6">Only force founders and leaders can manage divisions.</p>
      ...
      <div className="max-w-6xl mx-auto mb-8">
        <div className="flex flex-wrap justify-between items-start gap-4">
          <div>
            ...
            >
              <ChevronLeft className="w-4 h-4" /> Back to {force?.name}
            </Link>
            <h1 className="text-2xl md:text-3xl font-bold text-midnight-900">
              Division Management
            </h1>
            <p className="text-midnight-500 mt-1">
              Organize your force into specialized neighborhood divisions
            </p>
          </div>
      ...
      {divisions.length === 0 ? (
        <Card className="max-w-lg mx-auto text-center p-10">
          ...
          <p className="text-midnight-500 mb-6">
            Create divisions to organize your force by neighborhood or specialization.
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {divisions.map(division => (
            <Card key={division.id} className="hover:shadow-card-hover transition">
              ...
              <Link
                href={`/rescue-forces/${forceId}/divisions/${division.id}`}
