  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      if (!connections.has(missionId)) {
        connections.set(missionId, new Map());
      }
      connections.get(missionId).set(connectionId, {
        controller,
        lastPing: Date.now(),
      });
      ...
      const heartbeat = setInterval(() => {
        try {
          ...
          const conn = connections.get(missionId)?.get(connectionId);
          if (conn) {
      ...
      const cleanup = () => {
        clearInterval(heartbeat);
        connections.get(missionId)?.delete(connectionId);
