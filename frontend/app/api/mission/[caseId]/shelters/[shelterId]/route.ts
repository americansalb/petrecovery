 ...
 *
 * POST /api/mission/[caseId]/shelters/[shelterId] - Log call or send email
 * GET /api/mission/[caseId]/shelters/[shelterId] - Get shelter details with history
 *
...
/**
 * GET /api/mission/[caseId]/shelters/[shelterId]
 *
 * Get shelter details with contact history
 */
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ caseId: string; shelterId: string }> }
  ...
  try {
    ...
    const { caseId, shelterId } = await params;

    const shelter = await prisma.shelterContact.findFirst({
      where: { id: shelterId, caseId },
      include: {
        attempts: {
...
/**
 * POST /api/mission/[caseId]/shelters/[shelterId]
 *
...
export async function POST(
  ...
  try {
    ...
    if (body.action === 'call') {
      return handleCallLog(user.id, caseId, shelter, body, caseRecord.createdAt);
    } else if (body.action === 'email') {
      return handleEmailSend(user.id, caseId, shelter, body, caseRecord.createdAt);
    } else {
      return NextResponse.json(
