  ...
  try {
    ...
    for (const force of rescueForces) {
      try {
        ...
        const created = await prisma.rescueForce.create({
          data: {
            name: force.name,
        ...
        results.created.push({ name: force.name, id: created.id });
      } catch (err) {
        results.errors.push({ name: force.name, error: err.message });
      }
    }
    ...
    console.error('Error seeding forces:', error);
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json({ forces, count: forces.length });
  } catch (error) {
