  ...
  try {
    ...
    for (const squad of rescueSquads) {
      try {
        ...
        if (existing) {
          results.skipped.push(squad.name);
          continue;
        }
        ...
        results.created.push({ name: squad.name, id: created.id });
      } catch (err) {
        results.errors.push({ name: squad.name, error: err.message });
      }
    }
    ...
    console.error('Error seeding squads:', error);
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json({ squads, count: squads.length });
  } catch (error) {
