...
// GET /api/rescue-squads/[id]/nearby-missions - Get nearby cases for ALL squad members to see
  ...
  try {
    ...
    if (!membership) {
      return NextResponse.json(
        { error: 'Only rescue force members can view nearby cases' },
        { status: 403 }
      );
    }
    ...
    if (!squad || !squad.centerLatitude || !squad.centerLongitude) {
      return NextResponse.json({ error: 'Rescue Force location not configured' }, { status: 400 });
    }
