...
/**
 * GET /api/rescue-forces/[id]/hub
 *
 * Returns all data needed for the Force Hub in a single API call.
 * This aggregates force info, membership, divisions, cases, chat, etc.
 */
export async function GET(request, { params }) {
  try {
    ...
    const forceIdOrSlug = params.id;

    // Try to find force by ID first, then by city name (slug)
    let force = await prisma.rescueForce.findUnique({
      where: { id: forceIdOrSlug },
      include: {
        divisions: {
          where: { isActive: true },
          orderBy: { name: 'asc' },
    ...
    if (!force) {
      const cityName = forceIdOrSlug.replace(/-/g, ' ');
      force = await prisma.rescueForce.findFirst({
        where: {
    ...
    // Use the actual force ID for subsequent queries
    const forceId = force.id;

    console.log('[Hub Debug] Found force:', { id: forceId, name: force.name, city: force.city });
    ...
    // Get cases assigned to this force
    ...
    const cases = caseAssignments.map(assignment => {
      ...
      return {
        ...
        divisionId: assignCaseToDivision(c, force.divisions),
        petName: c.petName,
    ...
    const divisions = force.divisions.map(div => ({
      id: div.id,
      name: div.name,
    ...
    const allMembers = await prisma.rescueForceMember.findMany({
      where: {
        rescueForceId: forceId,
        isActive: true,
      },
    ...
    // Get chat messages (stored as ForceActivity with type CHAT_MESSAGE)
    ...
    const chatMemberships = chatAuthorIds.length > 0 ? await prisma.rescueForceMember.findMany({
      where: {
        rescueForceId: forceId,
    ...
    // Get announcements (stored as ForceActivity with type ANNOUNCEMENT)
...
Every share, every search, every kind word makes a difference. Together, we bring pets home!`;
    ...
    // Get help requests (ForceTask with type REQUEST)
    ...
    const requests = tasks.map(t => {
      ...
      return {
        ...
        title: t.title,
        body: t.description || '',
        divisionId: null, // ForceTask doesn't have divisionId field
    ...
    const onDutyCount = await prisma.rescueForceMember.count({
      where: {
        rescueForceId: forceId,
        isActive: true,
    ...
    const hubData = {
      force: {
        id: force.id,
        citySlug: force.city?.toLowerCase().replace(/\s+/g, '-') || force.id,
        cityName: force.city || 'Unknown City',
        state: force.state || '',
        country: force.country || 'US',
        displayName: force.name,
        description: force.description,
        photoUrl: force.photoUrl,
        slogan: force.slogan,
        zipCode: force.zipCode,
        memberCount: force._count.members,
        onDutyCount,
        centerLat: force.centerLatitude,
        centerLng: force.centerLongitude,
        customBoundary: force.customBoundary,
      },
      membership,
      divisions,
      cases,
    ...
    console.error('Error fetching force hub data:', error);
    return NextResponse.json(
