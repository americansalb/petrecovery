...
export async function PATCH(request, { params }) {
  try {
    ...
    const { id: forceId, memberId } = params;
    ...
    if (!targetMember || targetMember.rescueForceId !== forceId) {
      return NextResponse.json(
    ...
    await prisma.eventLog.create({
      data: {
        event_type: 'force.member_updated',
...
// DELETE /api/rescue-forces/[id]/members/[memberId] - Remove member from force
export async function DELETE(request, { params }) {
  try {
    ...
    await prisma.eventLog.create({
      data: {
        event_type: 'force.member_removed',
        correlation_id: crypto.randomUUID(),
        actor_user_id: session.user.id,
        actor_role: requesterMembership.role,
        resource_type: 'rescue_squad_member',
