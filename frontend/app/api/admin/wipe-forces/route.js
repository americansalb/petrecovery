...
// POST /api/admin/wipe-forces - Delete all rescue force data (ADMIN ONLY)
export async function POST(request) {
  const timestamp = new Date().toISOString();
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🗑️  [${timestamp}] WIPE FORCES REQUEST STARTED`);
  ...
  try {
    ...
    // Step 5: Delete force members
    console.log('\n🔄 Step 5: Deleting force members...');
    let membersResult;
    try {
      membersResult = await prisma.rescueForceMember.deleteMany({});
      console.log(`✅ Deleted ${membersResult.count} force members`);
    } catch (membersError) {
    ...
    try {
      ...
      console.error('❌ Error deleting forces:', squadsError);
      console.error('   Error details:', {
