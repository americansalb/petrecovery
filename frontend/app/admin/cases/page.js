...
export default async function AdminCasesPage() {
  ...
  // Basic role check - detailed permissions handled by middleware/layout
  ...
  const statusColors = {
    ...
    ACTIVE_SEARCH: 'bg-red-100 text-red-800',
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-red-50 p-4 rounded-lg shadow">
          <p className="text-red-600 text-sm">Active Search</p>
          <p className="text-2xl font-bold text-red-700">
            {cases.filter(c => c.status === 'ACTIVE_SEARCH').length}
          </p>
        </div>
        <div className="bg-green-50 p-4 rounded-lg shadow">
          <p className="text-green-600 text-sm">Resolved</p>
          <p className="text-2xl font-bold text-green-700">
            {cases.filter(c => ['RESOLVED', 'FOUND'].includes(c.status)).length}
          </p>
        </div>
      </div>
              ...
              <tr key={c.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium">{c.petName || 'Unknown'}</p>
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`px-2 py-1 rounded text-xs font-medium ${statusColors[c.status] || 'bg-gray-100 text-gray-800'}`}>
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  {c.squad ? (
                    <span className="text-sm">{c.squad.name}</span>
                  ) : (
