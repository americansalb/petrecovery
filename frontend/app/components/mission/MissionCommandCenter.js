...
/**
 * Mission Command Center - SAR Operations Interface
 *
...
export default function MissionCommandCenter({ missionId, missionNumber, onClose }) {
  ...
  const fetchMission = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/missions/${identifier}`);
  ...
  const fetchSightings = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        buildTimeline(missionData, sightingsList);
      }
    } catch (err) {
      console.error('Error fetching sightings:', err);
    }
  }, [missionData, buildTimeline]);
  ...
  const toggleChecklistItem = async (itemId) => {
    ...
    if (missionData?.id) {
      try {
        await fetch(`/api/missions/${missionData.id}/checklist`, {
          method: 'POST',
      ...
      <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col">
        ...
        <div className="bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 py-2 flex items-center justify-between safe-area-top">
          ...
          <div className="flex-1 text-center">
            <span className="text-white font-bold">{missionData?.petName || 'Unknown'}</span>
            {timeMissing && (
          ...
          <div className="bg-slate-900 border-b border-slate-800 p-3">
            <div className="flex items-center gap-3">
              {missionData?.petPhotoUrl ? (
                <img src={missionData.petPhotoUrl} alt={missionData.petName} className="w-14 h-14 rounded-xl object-cover" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-white truncate">{missionData?.petName}</span>
                  <span className="text-xs text-slate-500">{missionData?.petBreed}</span>
                </div>
                <p className="text-sm text-slate-400 truncate">
                  {missionData?.petColor} {missionData?.petSpecies?.toLowerCase()}
                </p>
                <p className="text-xs text-slate-500 truncate">{missionData?.lastSeenAddress}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-slate-900 to-transparent p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 bg-slate-900/80 backdrop-blur rounded-xl px-4 py-2">
                {missionData?.petPhotoUrl ? (
                  <img src={missionData.petPhotoUrl} alt="" className="w-10 h-10 rounded-lg object-cover" />
                ) : (
                  <div className="w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center">
                    {missionData?.petSpecies === 'DOG' ? '🐕' : '🐈'}
                  </div>
                )}
                <div>
                  <div className="font-bold text-white">{missionData?.petName}</div>
                  <div className="text-xs text-slate-400">{missionData?.petBreed} • {missionData?.petColor}</div>
                </div>
                {timeMissing && (
            ...
            {activeView === 'info' && (
              <div className="space-y-4">
                <div className="bg-slate-800/50 rounded-xl p-4">
                  ...
                  <p className="text-white">{missionData?.lastSeenAddress || 'Unknown'}</p>
                ...
                <div className="bg-slate-800/50 rounded-xl p-4">
                  ...
                  <p className="text-white text-sm">{missionData?.petDescription || 'No description provided'}</p>
                </div>

                {missionData?.ownerPhone && (
                  <div className="bg-slate-800/50 rounded-xl p-4">
                    <h4 className="text-slate-400 text-xs font-semibold uppercase mb-3">Contact</h4>
                    <a href={`tel:${missionData.ownerPhone}`} className="text-cyan-400 font-semibold">
                      📞 {missionData.ownerPhone}
                    </a>
                  </div>
                )}
      ...
      <div className="w-80 bg-slate-900 border-r border-slate-800 flex flex-col">
        ...
        <div className="p-4 border-b border-slate-800">
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-4 border border-slate-700">
            <div className="flex items-start gap-4">
              {missionData?.petPhotoUrl ? (
                <img src={missionData.petPhotoUrl} alt={missionData.petName} className="w-20 h-20 rounded-xl object-cover" />
              ) : (
              ...
              <div className="flex-1">
                <h2 className="text-xl font-bold text-white">{missionData?.petName || 'Unknown'}</h2>
                <p className="text-slate-400 text-sm">{missionData?.petBreed}</p>
                <p className="text-slate-500 text-sm">{missionData?.petColor} • {missionData?.petSize}</p>
              </div>
            </div>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="bg-slate-800/50 rounded-xl p-3">
            ...
            <div className="text-white text-sm">{missionData?.lastSeenAddress || 'Unknown'}</div>
          </div>

          <div className="bg-slate-800/50 rounded-xl p-3">
            <div className="text-xs text-slate-500 mb-1">Description</div>
            <div className="text-white text-sm line-clamp-3">{missionData?.petDescription || 'No description'}</div>
          </div>
        </div>
