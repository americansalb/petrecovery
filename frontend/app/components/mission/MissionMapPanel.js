...
/**
 * Mission Map Panel - Center panel of Command Center
 *
...
export default function MissionMapPanel({ missionData, searchAreas = [], onSightingClick }) {
  ...
  useEffect(() => {
    ...
    if (!missionData?.lastSeenLatitude || !missionData?.lastSeenLongitude) return;
    ...
    import('leaflet').then((L) => {
      ...
      const lastSeenTime = missionData.lastSeenAt ? new Date(missionData.lastSeenAt) : new Date(missionData.createdAt);
      const hoursElapsed = Math.max(1, (Date.now() - lastSeenTime.getTime()) / 3600000);

      // Get search radius based on pet type and time
      const radius = calculateSearchRadius(hoursElapsed, missionData.petSpecies);
      const center = [missionData.lastSeenLatitude, missionData.lastSeenLongitude];
  ...
  }, [missionData, showProbabilityZones]);
  ...
  const fetchNearbyResources = useCallback(async () => {
    ...
    try {
      ...
      const lat = missionData.lastSeenLatitude;
      const lon = missionData.lastSeenLongitude;
        ...
        marker.bindPopup(`
          <div class="p-2 min-w-[180px]">
            ...
            <div class="text-xs text-cyan-600 mt-2">
              Check if they've seen ${missionData?.petName || 'the pet'}!
            </div>
          </div>
        `, { className: 'custom-popup' });
  ...
  }, [nearbyResources, showResources, missionData?.petName]);
    ...
    if (missionData?.lastSeenLatitude && missionData?.lastSeenLongitude) {
        ...
        html: `
          <div class="relative">
            ...
            <div class="relative w-12 h-12 bg-gradient-to-br from-red-500 to-rose-600 rounded-full shadow-lg shadow-red-500/50 flex items-center justify-center border-2 border-white">
              <span class="text-xl">${missionData.petSpecies === 'DOG' ? '🐕' : missionData.petSpecies === 'CAT' ? '🐈' : '🐾'}</span>
            </div>
          </div>
        `,
        iconSize: [48, 48],
      ...
      lastSeenMarker.bindPopup(`
        <div class="p-2">
          ...
          <div class="text-sm text-gray-600">${missionData.lastSeenAddress || 'Unknown address'}</div>
    ...
    <div className="h-full flex flex-col bg-slate-900">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => {
              if (mapInstanceRef.current && missionData?.lastSeenLatitude) {
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bottom-4 left-4 bg-slate-900/90 backdrop-blur rounded-xl p-3 border border-slate-700/50 z-10 max-w-[200px]">
          <div className="text-xs space-y-2">
            ...
            {showProbabilityZones && missionData?.lastSeenLatitude && (
              <>
        ...
        {/* No location warning */}
        {!missionData?.lastSeenLatitude && !loading && (
