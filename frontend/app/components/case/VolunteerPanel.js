 ...
 *
 * Real-time volunteer coordination for Case Command Center:
...
export default function VolunteerPanel({
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/cases/${caseData.id}/tasks`);
      if (res.ok) {
        const data = await res.json();
        setTasks(data.tasks || []);
      }
...
// Task List Component
function TaskList({ tasks, caseData, currentUserId, isCoordinator, showNewTask, setShowNewTask, onTaskUpdate }) {
