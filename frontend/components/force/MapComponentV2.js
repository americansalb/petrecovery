...
export default function MapComponentV2({
  ...
  useEffect(() => {
    ...
    }).setView([force.centerLat || 41.8781, force.centerLng || -87.6298], 13);
  ...
  useEffect(() => {
    if (!mapInstanceRef.current || !force.cityName) return;
    ...
    const drawBoundary = async () => {
      try {
        ...
        // First, check if we have a stored boundary in the force
        if (force.customBoundary) {
          try {
            geometry = typeof force.customBoundary === 'string'
              ? JSON.parse(force.customBoundary)
              : force.customBoundary;
            console.log('[Map] Using stored boundary for', force.cityName);
          } catch (e) {
        ...
        if (!geometry) {
          const cityName = force.cityName;
          const state = force.state || '';
          const country = force.country || 'US';
  ...
  }, [force.cityName, force.state, force.customBoundary]);
