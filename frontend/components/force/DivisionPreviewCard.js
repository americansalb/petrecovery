 ...
 *
 * Appears when user clicks a division chip on force page
...
export default function DivisionPreviewCard({
  ...
  const handleViewFullPage = () => {
    router.push(`/rescue-forces/${forceId}/divisions/${division.id}`);
  };
        ...
        >
          ...
          <div className="p-8 border-b border-slate-700/50">
            <div className="flex items-start gap-4">
              <div className="p-4 rounded-xl bg-gradient-to-br from-flash-500/20 to-flash-400/20 border-2 border-flash-500/40 shadow-lg shadow-flash-500/20">
                <Users className="text-flash-300" size={28} />
              </div>
              <div>
                ...
                <p className="text-slate-400 text-base font-medium">
                  Division · Part of force coverage area
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {recentMissions.map(c => {
                  ...
                  return (
                    <div
                      key={c.id}
                      className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-br from-slate-800/40 to-slate-900/40 backdrop-blur-sm border border-slate-700/50 hover:border-flash-500/40 hover:shadow-lg hover:shadow-flash-500/10 transition-all duration-200"
                    >
...
function StatBox({ icon: Icon, value, label, color }) {
  const colorClasses = {
    ...
    cyan: 'text-flash-300 bg-gradient-to-br from-cyan-500/15 to-cyan-400/5 border-flash-500/40 shadow-cyan-500/10',
