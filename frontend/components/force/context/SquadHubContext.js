...
/**
 * ForceHubContext - State Management for Force Hub
 *
 * Manages:
...
const ForceHubContext = createContext(null);

export function ForceHubProvider({ children, initialData, forceId }) {
  ...
  const toggleOnDuty = useCallback(async () => {
    ...
    if (forceId) {
      try {
        const res = await fetch(`/api/rescue-forces/${forceId}/toggle-duty`, {
          method: 'POST',
        });
        if (!res.ok) {
          // Revert on error
          setData(prev => ({
            ...
            force: {
              ...prev.force,
              onDutyCount: wasOnDuty ? prev.force.onDutyCount + 1 : prev.force.onDutyCount - 1,
            },
          }));
        }
      } catch (err) {
  ...
  }, [data.membership.isOnDuty, forceId]);
  ...
  const joinSquad = useCallback(async () => {
    ...
    setData(prev => ({
      ...
      force: {
        ...prev.force,
        memberCount: prev.force.memberCount + 1,
      },
    }));
    ...
    if (forceId) {
      try {
        const res = await fetch(`/api/rescue-forces/${forceId}/join`, {
          method: 'POST',
        });
        if (!res.ok) {
          // Revert on error
          setData(prev => ({
            ...
            force: { ...prev.force, memberCount: prev.force.memberCount - 1 },
          }));
        }
      } catch (err) {
        console.error('Failed to join force:', err);
      }
    }
  }, [forceId]);
  ...
  const helpOnCase = useCallback(async (missionId) => {
    ...
    if (forceId) {
      try {
        const res = await fetch(`/api/rescue-forces/${forceId}/cases/${missionId}/help`, {
          method: 'POST',
        });
        if (!res.ok) {
          // Revert on error
  ...
  const helpOnRequest = useCallback(async (requestId) => {
    ...
    if (forceId) {
      try {
        await fetch(`/api/rescue-forces/${forceId}/requests/${requestId}/help`, {
          method: 'POST',
        });
      } catch (err) {
  ...
  const leaveCase = useCallback(async (missionId) => {
    ...
    if (forceId) {
      try {
        await fetch(`/api/rescue-forces/${forceId}/cases/${missionId}/help`, {
          method: 'DELETE',
        });
      } catch (err) {
  ...
  const postRequest = useCallback(async (title, body, divisionId = null, missionId = null) => {
    ...
    if (forceId) {
      try {
        const res = await fetch(`/api/rescue-forces/${forceId}/requests`, {
          method: 'POST',
  ...
  const postAnnouncement = useCallback(async (title, content, divisionId = null, isPinned = false) => {
    ...
    if (forceId) {
      try {
        const res = await fetch(`/api/rescue-forces/${forceId}/announcements`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <ForceHubContext.Provider value={value}>
      {children}
    </ForceHubContext.Provider>
  );
}

export function useForceHub() {
  const context = useContext(ForceHubContext);
  if (!context) {
    throw new Error('useForceHub must be used within a ForceHubProvider');
  }
  return context;
}

export default ForceHubContext;
