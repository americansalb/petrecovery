...
export default function PostFeed({
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/rescue-forces/${forceId}/posts?${params}`);
      if (res.ok) {
  ...
  const handleVote = async (postId, voteType, isComment = false) => {
    try {
      const endpoint = isComment
        ? `/api/rescue-forces/${forceId}/comments/${postId}/vote`
        : `/api/rescue-forces/${forceId}/posts/${postId}/vote`;
  ...
  const handleComment = async (postId, content, replyTo = null) => {
    try {
      const res = await fetch(`/api/rescue-forces/${forceId}/posts/${postId}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content,
