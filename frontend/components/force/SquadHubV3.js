...
/**
 * ForceHubV3 - Redesigned with consistent panel-based approach
 *
 * Matches Mission Control V2 design language:
 * - Always-visible map (zoomed out to show all cases)
 * - Expandable panels for Cases, Members, Force Chat
...
export default function ForceHubV3({ initialData, forceId }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      onDuty: force.onDutyCount || 0,
    };
  }, [allCases, force]);

  // Map center - calculate from all cases or use force location
  const mapCenter = useMemo(() => {
    ...
    // Default to Austin, TX or force location
    return force.latitude && force.longitude
      ? [force.latitude, force.longitude]
      : [30.2672, -97.7431];
  }, [allCases, force]);
  ...
  const handleJoinCase = async (missionData, e) => {
    e.stopPropagation();
    try {
      const res = await fetch(`/api/rescue-forces/${forceId}/cases/${missionData.id}/help`, {
        method: 'POST',
      });
      if (res.ok) {
        router.push(`/mission-control?mission=${missionData.missionNumber || missionData.id}`);
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-white">{force.displayName || 'Rescue Force'}</h1>
              <p className="text-slate-400 text-sm flex items-center gap-1">
                <MapPin size={14} />
                {force.cityName || 'Unknown location'}
              </p>
            </div>
          </div>
      ...
      {/* Map - Always visible, zoomed out to show force coverage */}
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-7xl mx-auto">
          ...
          >
            <MembersPanel force={force} membership={membership} forceId={forceId} />
          </ExpandablePanel>
          ...
          >
            <SquadChatPanel forceId={forceId} messages={chatMessages} />
          </ExpandablePanel>
        </div>
      </div>
    </div>
...
// ============================================================================
// CASES PANEL - List of all force cases
// ============================================================================
function CasesPanel({ cases, forceId, onCaseClick, onJoinCase }) {
...
// ============================================================================
// MEMBERS PANEL - Force members
// ============================================================================
function MembersPanel({ force, membership, forceId }) {
...
// ============================================================================
// FORCE CHAT PANEL - Force-wide messaging
// ============================================================================
function ForceChatPanel({ forceId, messages: initialMessages }) {
    ...
    <div className="flex flex-col h-80">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 mb-4">
        {messages.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-500 text-sm mt-1">Say hello to your force!</p>
          </div>
        ) : (
          messages.map((msg, i) => (
      ...
      <form onSubmit={sendMessage} className="flex gap-2">
        <input
          ...
          placeholder="Message your force..."
