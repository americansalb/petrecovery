...
export default function PhotoUploadModal({ isOpen, onClose, onUpload, forceId }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Then, update force with new photo URL
      const updateResponse = await fetch(`/api/rescue-forces/${forceId}/photo`, {
        method: 'POST',
        headers: {
      ...
      if (!updateResponse.ok) {
        throw new Error(updateData.error || updateData.details || 'Failed to update force photo');
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-2 border-flash-500/30 rounded-2xl shadow-[0_0_60px_rgba(250,204,21,0.3)] max-w-md w-full p-8">
        ...
        {/* Header */}
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white mb-2">Upload Force Photo</h2>
