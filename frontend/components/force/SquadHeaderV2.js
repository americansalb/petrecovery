...
/**
 * ForceHeaderV2 - Modern, polished force header
...
export default function ForceHeaderV2({
  force,
  divisions,
  stats,
  onDivisionClick,
  ...
  const cityName = force.cityName || 'Unknown City';
  ...
  const handleJoinSquad = async () => {
    try {
      ...
      console.error('Failed to join force:', error);
    }
  };

  const squadPhoto = force.photoUrl || null;
  const squadDescription = force.description || `A dedicated community of volunteers protecting lost and found pets in ${cityName}.`;
  const zipCode = force.zipCode || '';
  const customSlogan = force.slogan || (state && zipCode ? `${cityName}, ${state} ${zipCode}` : `${cityName}${state ? ', ' + state : ''}`);
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex flex-wrap gap-2">
          {isDivisionPage ? (
            <>
              <button
                onClick={() => router.push(`/rescue-forces/${force.id}`)}
              ...
              {divisions.filter(d => d.id !== currentDivisionId).map(div => (
                <DivisionChip
                  key={div.id}
                  active={false}
                  onClick={() => router.push(`/rescue-forces/${force.id}/divisions/${div.id}`)}
                  label={div.name}
                />
              ))}
            </>
