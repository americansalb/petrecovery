...
/**
 * MapComponentApple - Apple MapKit implementation for Squad Hub
 *
 * Renders the interactive Apple Map with case pins, squad coverage area,
 * and case markers with photo/emoji support.
 *
 * Features:
 * - Dark mode Apple Maps
 * - Custom case markers with photos
 * - Squad coverage circle overlay
...
export default function MapComponentApple({
  ...
  // Compute center from squad data
  const mapCenter = useMemo(() => ({
    lat: squad.centerLat || squad.centerLatitude || 41.8781,
    lng: squad.centerLng || squad.centerLongitude || -87.6298,
  }), [squad.centerLat, squad.centerLatitude, squad.centerLng, squad.centerLongitude]);
  ...
  useEffect(() => {
    ...
    if (mapCenter.lat && mapCenter.lng) {
      const radiusMeters = (squad.coverageRadiusMiles || 10) * 1609.34;
  ...
  }, [mapReady, map, mapCenter, squad.coverageRadiusMiles, addCircle, removeOverlay]);
