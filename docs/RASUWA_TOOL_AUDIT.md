...
- **Review hardening on the letter refresh (Codex findings, PR #228):**
  ...
  the next refresh.
- **A real path for every country on the list (founder ask,
  2026-09-01: "add compatibility for all of the countries where
  people are missing, as much as possible"):** every nationality on
  the letter now has a working route through the tool. Nepal joined
  the country guides (House of Representatives and Ministry of
  Foreign Affairs links), and because Nepal's own government runs the
  search, its letters flip the frame: instead of pressing a foreign
  ministry to press Nepal, they ask for the offered international
  technical support to be accepted and put to work, with the call
  guidance adjusted to match. Everywhere else, the international
  letters and the call guidance now name the writer's actual foreign
  ministry (DFAT, FCDO, MFA Singapore, MEA India, the French and
  South African ministries) instead of "our foreign ministry", and
  letters carry the country's own stake from the list ("one of the 8
...
- **The emergency-disclosure ask (founder direction, 2026-09-02: get
  the missing people's last known locations through provider
  emergency disclosure):** every letter, person and general, on every
  path, now carries a fourth ask. Under 18 U.S.C. 2702, Google,
  Apple, Meta, and the phone carriers may hand a person's last known
  device location to a governmental entity in a danger-of-death
  emergency, and missing persons qualify under the providers' own
  policies; the request must come from law enforcement, which is why
  the letters ask for a missing person case first. US letters ask
  the FBI (the general letter, the FBI and the Department of
  Justice); Canadian letters the RCMP or local police; international
  letters the national police; Nepal letters Nepal Police, with
  every result routed to the officers coordinating the search. The
  US call guidance adds the family-side move: any local police
  department can open the case and send the emergency disclosure
  request today. Template v7 rebuilds older hand-edited drafts.
- **Anyone can ask for a mistake to be reviewed (founder ask,
  2026-09-02):** /rasuwa/correction is a public form: pick who it is
  about (or the letter overall), say what is wrong and what it should
  say instead, optionally leave a way to be reached. No account.
  Requests land in a new RasuwaCorrection table (rate limited,
  clipped by corrections.js) and show up on the task force board in a
  new Corrections section with Mark handled and Reopen, the same
  conditional-update pattern as the needs. The site never edits the
  list from a request: the families fix the letter document at its
  source and the next regeneration follows it, which is what the form
  and the board both say. Doors on every surface where details show:
