# Squads, Divisions & Cases – Product Vision

PetRecovery.org should feel like the **best search & rescue UI in the world**, starting with lost pets. This doc nails down what **Squads**, **Divisions**, and **Cases** *mean* so we stop drifting.
...
- Divisions are mainly about **who gets notified** and **which map area a case belongs to**.
...
- Has:
  - A pet owner (case "leader")
  - A set of helpers (squad members who accepted the case)
...
- Shows divisions, case queues, map of active cases, and squad community activity.

**Case Command Center (Layer B – "Mission view")**
- Deep, focused page per case.
...
COUNTRY: United States
  └── City (Chicago)
        └── Squad (Chicago Rescue Squad)
              ├── Divisions
              │    ├── Lakeview
              │    │     ├── Case #CHI-LKV-0001 (Max)
              │    │     └── Case #CHI-LKV-0002 (Whiskers)
              │    ├── South Side
              │    │     └── Case #CHI-SS-0003
              │    └── Logan Square
              │          └── Case #CHI-LSQ-0004
...
2. **Cases live in exactly one primary Division.**
   We can later support "spillover" visibility into neighboring divisions, but the case always has a home.
...
- **Being in a Squad** means:
  - You can see all public cases in that city.
...
- **Being in a Division (home or extra)** means:
  - You get priority notifications for new cases in that division.
  - Those cases appear at the top of your personal "Help Queue."

### 3.3. Case visibility & escalation (Lakeview example)

**Pet goes missing in Lakeview:**

1. **Initial scope (Division-first):**
   - Case is created with `division = Lakeview`.
   - Notifications go to:
...
3. **Squad-wide escalation:**
   - For high-priority cases (service animals, extreme weather) or manual override:
...
- Everyone in the squad receives notifications for all cases.
...
| Role | Description |
|------|-------------|
| **Pet Owner (Case Leader)** | The person who created the case / is responsible for the pet |
...
| Escalate case to full squad | ✅ | ✅ (if assigned) | ✅ | ✅ |

**Note:** "Case Leader" is a property of the case, not a global role. In many flows the pet owner is the leader, but a shelter or vet could also create cases.
...
- Let people join, go on duty, pick divisions, and grab cases to help with.
...
2. **Left panel: Case queue**
   - Tabs: `Incoming`, `Active`, `Reunited`.
   - Big, visual case cards with status, time, and "Help with this pet" button.

3. **Center: City map**
   - Pins colored by status, grouped by division.
   - "Use my location" to snap to nearest division and relevant cases.

4. **Right: Squad activity lane**
   - Division / city-wide chat, announcements, quick checklists.

### 5.2. Layer B – Case Command Center (Mission View)
...
1. **Left:** Case summary and people (owner, helpers, status, reward).
...
3. **Step-by-step flows.**
   - Wizards for complex tasks like initial case setup and marking searched areas.
...
- It wires together:
  ...
  - Case queue (which cases need help)
  - The first version of the map mental model.
- It gives us a natural entry point into the Case Command Center:
  - Tap a case card or map pin → open the mission UI.

### 7.2. Second page to build: Case Command Center

Once the hub exists, we immediately design & implement the Case Command Center as the "hero" experience:
- The hub gets people into the right case,
- The case page delivers the wow.
...
2. **Cases belong to exactly one primary Division** and one Squad.
3. **Membership is at Squad level;** users tune divisions for notifications.
4. **Divisions are about who gets notified first** and where on the map the case lives.
5. **Two main UI layers:**
   - Squad / Division Hub (community & queues)
   - Case Command Center (deep coordination)
...
| **Member, Off Duty** | Default state. Can see cases, manually accept. Low-frequency notifications. |
...
- Prioritized for notifications for new cases in chosen divisions
- If location sharing allowed, approximate location shown on map
- Ephemeral: auto-timeout after ~12 hours

---

## 10. Case Queue Tab Definitions

| Tab | Shows | Sorted By |
|-----|-------|-----------|
| **Incoming** | Cases where user is NOT a helper, status is NEW/ACTIVE/IN_PROGRESS, still seeking helpers | Urgency → Recency |
| **Active** | All non-reunited cases (shows "You're helping" badge if user is helper) | Urgency → Recency |
| **Reunited** | Cases with status REUNITED | Reunion time (recent first) |
...
- Each chip shows active case count: `Lakeview (3)`
- Selecting a division filters: case queue, map, activity lane
...
| Viewport | Layout |
|----------|--------|
| **Mobile** | Tabbed: Cases / Map / Squad |
| **Tablet** | 2-panel: Cases + Map (activity in drawer) |
| **Desktop** | 3-panel: Cases / Map / Activity |
