...
### Where on Earth (geo game, 2026-09)
...
| `geo/config/route.js` | GET | Where on Earth: which providers are set up, the Google browser key (public by design, referrer-locked), daily seed, country list. Never a server key | public; middleware 30/min | none |
...
| `/geo/script/play` | `ScriptMap` (MapKit JS). No imagery pane: the round is a sentence. The reveal draws the language's heartlands as circle overlays, because the answer is an area |
