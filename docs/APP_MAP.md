...
| `/geo/play` | `AppleLookAroundPane` + `AppleGuessMap` (MapKit JS Look Around). Apple is the only imagery: Google went in 2026-09 (founder direction), along with `GoogleStreetViewPane`, `GoogleGuessMap` and `app/geo/lib/googleMaps.js`. A Not Earth round swaps the pane for `NotEarthPane` (a NASA panorama) and nothing else changes |
