...
| `geo/round/route.js` | POST | Draws seeded candidate points per mode, probes the free Street View metadata endpoint server-side (official outdoor imagery only), returns a panorama id + AES-GCM sealed answer token; Apple rounds return candidate coordinates to try in the browser | public; middleware 40/min | none |
