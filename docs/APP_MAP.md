...
The whole `/geo` segment is immersive (own header/footer in `app/geo/layout.js`, ReunitePets link as the way out) so it can also be served on a game domain of its own: `GEO_DOMAINS` in `middleware.js` (docs/GEO.md).
