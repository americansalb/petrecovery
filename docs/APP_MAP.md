...
| `/lost-pet/[location]` | SEO city landing page: found and missing pets in the town, a shelters link, the local Rescue Force, what to do | server wrapper + client | none | `location` = `city-st` slug, e.g. `austin-tx`; parsed by `formatLocationSlug`, not DB-validated (cities from `app/lib/uscities.full.json`/`CityCache`) |
...
| route | purpose | client/server | auth | dynamic params & source |
|---|---|---|---|---|
| `/cases/[caseNumber]` | Canonical public case landing page (photo and facts; I've seen / call / share; last-seen map with sightings; sightings and updates; other ways to help; flyer kit; owner panel with mark-as-reunited) | server wrapper + client | none (owner extras when logged in) | `Case.caseNumber` (e.g. `"CHI-2024-001847"`) |
...
| `/rescue-forces/[id]/members` | Member list; leaders change role/division and remove; everyone can leave; "Become the leader" on a force with no founder/leader | server guard + client `MembersClient` | member | `RescueForce.id` |
| `/rescue-forces/[id]/settings` | Photo, description, taking new reports (honored by `api/reports/create`), pets, times, equipment | server guard + client `SettingsClient` | founder/leader (`leadersOnly`) | `RescueForce.id` |
| `/rescue-forces/[id]/divisions` | Division list; leaders add/edit/delete and set an area (ZIP centre + radius, `app/lib/zipCenter.js`) | server guard + client `DivisionsClient` | member | `RescueForce.id` |
| `/rescue-forces/[id]/divisions/[divisionId]` | One division: pets matched to its area (`forcePublic` `zoneId`), its members | server | member | `RescueForce.id` + `Division.id` belonging to that force |
...
### Probably Earth (geo game, 2026-09)

| Route | Purpose | Type | Auth | Data |
|---|---|---|---|---|
| `/geo` | The front door: a turning Natural Earth globe you can spin, one Play button, and a row of four text links (Play with friends, Daily, Rankings, Script). No settings and no choices on the way in - the lobby that asked for imagery, mode, rounds, timer, format and radius was deleted (founder, 2026-09-16) and every mode it carried now lives on `/geo/leaderboard`. Full-screen takeover with its own small header | server layout (static share card) + client page | public | none (localStorage only) |
...
| `/geo/script` | The script game's page (docs/GEO.md, "Script"): read a sentence, pin where that language is spoken. Rounds and timer only; every game draws from the one pool, which is never listed or counted | server layout (own share card) + client page | public | none |
| `/geo/script/play?...` | A script game. The answer is a linguistic region rather than a point, so a pin is scored against the language's heartlands and the reveal draws them. No imagery provider and no key, so it never touches the play meter. Immersive route; the X returns to `/geo/script` | client | public | none |
| `/geo/rooms` | Multiplayer room browser: open a room (classic or duel, places, rounds, timer, format, public/private), join by code, public rooms active in the last 20 min. Apple Look Around is the only imagery | client | public | `GeoRoom` (list) |
| `/geo/room/[code]` | A room: join with a name (anonymous player token per room, hashed), lobby, rounds on the server clock, reveal with everyone's pins, duel HP, reactions, standings with rating changes, rematch. Polls `/api/geo/rooms/:code`; `generateMetadata` unfurls the room | server wrapper + client | public | `GeoRoom*` |
| `/geo/leaderboard` | Glicko ladders (classic, duel, ranked solo) with your own row, AND the rest of the game: Ranked, the daily challenge and the weekly cup with their boards (`Contests.js`), then country streak (`OtherModes.js`; one continent and one country were retired 2026-09-23 to keep coverage secret). These were the deleted lobby's, and this is the page that inherited them | client | public | `GeoRating`, `GeoProfile` |
| `/geo/me` | Your profile: name, rating, points, country badges, today's play meter, recent points, and the cosmetics shop (docs/GEO.md, "Points and cosmetics") | client | public (needs the browser's profile) | `GeoProfile`, `GeoBadge`, `GeoLedger`, `GeoUnlock` |

Chrome: the room browser, rankings, profile, script lobby and share pages are ordinary pages under the universal bar with the game's subtabs below it (`app/geo/components/GeoHeader.js`, `sticky top-16`: Play, Script, Rooms, Rankings, Daily, Profile); only `/geo` (exact), `/geo/play`, `/geo/room/` and `/geo/script/play` are immersive (`app/lib/geo/site.js`), each with a way back out. The trailing slash on `/geo/room/` is load-bearing: without it the prefix also swallowed `/geo/rooms`, the LIST of rooms, which then rendered with no navigation at all. A build with `NEXT_PUBLIC_SITE=geo` is the game's own site (no pet chrome, the game's header, non-game paths redirect to the pet site); `GEO_DOMAINS` in `middleware.js` gives the same redirects to a host on the shared deployment (docs/GEO.md, "Hosting on another domain").
...
| `geo/config/route.js` | GET | Probably Earth: which providers are set up, the Google browser key (public by design, referrer-locked), daily seed, country list. Never a server key | public; middleware 30/min | none |
| `geo/round/route.js` | POST | Runs the play meter first (docs/GEO.md: free Google rounds per day, prepaid rounds, ceilings, speed, site budget; 429 with a code on refusal), then draws seeded candidate points per mode, probes the free Street View metadata endpoint server-side (official outdoor imagery only), returns a panorama id + AES-GCM sealed answer token; Apple rounds return candidate coordinates to try in the browser. Charged only once imagery was found | public; middleware 40/min | `GeoUsage`, `GeoProfile` |
| `geo/guess/route.js` | POST | Opens the sealed token, scores the guess (`5000 e^(-10d/size)`) or judges the country for streaks, reveals the answer | public; middleware 60/min | none |
| `geo/og/route.js` | GET | 1200x630 share-card PNG from a share code (satori + resvg, bundled fonts); falls back to the logo | public; middleware 30/min | none |
| `geo/rooms/route.js` | GET, POST | Public rooms / open a room (rate-limited in-route); returns the host's player token | public; middleware 180/min (whole prefix) | `GeoRoom`, `GeoRoomPlayer` |
| `geo/rooms/[code]/route.js` | GET, POST | Room state for the `x-geo-player` token (spectator without) / `join`, `start`, `locate` (Apple rooms: which offered place has imagery), `guess`, `next`, `react`, `leave`, `rematch`. Every call ticks the room's clock; transitions are version-claimed | public | `GeoRoom*` |
| `geo/auth/request/route.js` | POST | Ask for a sign-in link. Answers identically whether or not the address has an account, so it cannot be used to enumerate people; the token is stored only as a SHA-256 | public; middleware 5/min | `GeoLoginToken` |
| `geo/auth/verify/route.js` | GET | Follow the link: burns it, creates or finds the `GeoAccount`, binds this browser's profile to it, sets the sealed session cookie, redirects to `/geo/me` | public; middleware 20/min | `GeoAccount`, `GeoLoginToken`, `GeoProfile` |
| `geo/auth/signout/route.js` | POST | Clears the game's two cookies. The browser keeps its play token, so signing out leaves you playing anonymously | public; middleware 60/min | none |
| `geo/auth/me/route.js` | GET | The signed-in email or nothing. Creates nothing | public; middleware 60/min | none |
| `geo/script/round/route.js` | POST | One round of the script game: a sentence from the server-only corpus, the writing system to render it in, and an AES-GCM sealed token naming the language. No provider, no key, no meter: a text round has no marginal cost | public; middleware 90/min | none |
| `geo/script/guess/route.js` | POST | Opens the sealed token and scores the pin against the language's heartland regions rather than a country, then reveals every region it is spoken in so the map can draw them. Unranked: no points, no daily board | public; middleware 120/min | none |
| `geo/script/report/route.js` | POST | "Something wrong?" from the Script answer screen: kind (area, hint, language, other), an optional note and the pin. The language and text come from the round's sealed token; the same person's repeat within a day is stored once (docs/GEO.md, "Players can say an answer is wrong") | public; middleware 10/min | `GeoScriptReport` |
| `geo/admin/reports/route.js` | GET, POST | Open Script reports piled up by language and kind, biggest first, with the latest notes; POST closes a pile | game admin (`requireAdmin`); middleware 120/min | `GeoScriptReport` |
| `geo/guess/route.js` (daily) | POST | Besides scoring, a daily round is recorded on the day's board for the request's profile, first guess per round only (docs/GEO.md, "The daily challenge") | public; middleware 60/min | `GeoChallengeRound`, `GeoChallengeEntry` |
| `geo/daily/route.js` | GET | The daily's board: finished players ranked by total (earlier finish first on a tie), counts, and your own row with its rank (`x-geo-profile`); `?date=` for a past day | public; middleware 30/min | `GeoChallengeEntry`, `GeoProfile` |
| `geo/shop/route.js` | GET, POST | The cosmetics shop: catalog with owned/usable/affordable flags and what is worn; `buy` takes points through the ledger and wears the item, `equip` wears something owned or unlocked by rating tier | public with a profile; middleware 60/min | `GeoUnlock`, `GeoLedger`, `GeoProfile` |
| `geo/cup/route.js` | GET | The weekly cup's board: finished players ranked, counts, when the week ends, the prizes, your row; pays out a finished week's prizes on first view (docs/GEO.md, "The weekly cup") | public; middleware 30/min | `GeoChallengeEntry`, `GeoChallengeFinal`, `GeoLedger` |
| `geo/profile/route.js` | POST | Rating profile: anonymous token (`x-geo-profile`, hashed) or the signed-in account, bound on first meeting | public; middleware 30/min | `GeoProfile`, `GeoRating`, `GeoMatchResult` |
| `geo/leaderboard/route.js` | GET | Ladder table (min 3 rated games) plus the asker's row | public; middleware 30/min | `GeoRating` |
...
### 6.9b Probably Earth (geo game)

`GeoRoom` (code, name, visibility, status/phase, variant, config JSON incl. seed, roundIndex, phaseEndsAt, version for claimed transitions, reactions JSON, retries, rematchCode, ratedAt) → `GeoRoomPlayer` (tokenHash, name, colour, host flag, score/hp/eliminated/roundWins, profileId, ratingBefore/After, placement) → `GeoRoomRound` (`@@unique([roomId, index])`, panorama id + answer + country; `candidates` JSON holds the places offered to browsers in an Apple room) → `GeoRoomGuess` (`@@unique([roundId, playerId])`). Accounts and ratings: `GeoAccount` (email unique) and `GeoLoginToken` (tokenHash unique, the emailed sign-in link, hashed, burned on use, carrying the `profileId` that asked for it) are the game's own identity, unrelated to the pet site's `User`; `GeoProfile` (tokenHash unique, `accountId` unique, `userId` dead and unread) → `GeoRating` (`@@unique([profileId, ladder])`, the all-time row) and `GeoSeasonRating` (`@@unique([profileId, ladder, season])`, one row per three-month season, carried in softly, docs/GEO.md "Seasons"), both Glicko rating + rd with games/wins/podiums/peak/streak and `GeoMatchResult` (`@@unique([roomId, profileId])`, with the season). `GeoRoundCache` keeps resolved seeded rounds for a day. `GeoChallengeRound` (`@@unique([profileId, key, index])`) and `GeoChallengeEntry` (`@@unique([profileId, key])`, total, rounds, finishedAt) are the shared boards, keyed `daily:YYYY-MM-DD` and `cup:YYYY-Www`; `GeoChallengeFinal` marks a cup week whose prizes were paid. `GeoLedger` (`@@unique([profileId, ref])`, earn/spend rows), `GeoUnlock` (`@@unique([profileId, itemId])`) and `GeoBadge` (`@@unique([profileId, countryCode])`, best km) are the points economy, with `GeoProfile.points` and `GeoProfile.equipped` (item ids per slot) alongside; `GeoRoomPlayer.pointsEarned` shows a game's points in the standings. `GeoUsage` (`@@unique([subject, day, provider])`) is the play meter: rounds per profile, hashed address, or the whole site, per UTC day per imagery provider, split into free and prepaid, plus the free room games used, the free daily/cup rounds used, and the panorama loads bought (a driven Kidnapped round is more than one); `GeoProfile.paidRounds` is the prepaid balance (quota packs, no subscriptions); `GeoRoomPlayer.ipHash` and `entry` record how a room seat was paid (free game, prepaid, apple, or only counted). `GeoScriptReport` holds what players said was wrong on a Script answer (language and text from the sealed round, kind, note, pin, a hashed address cleared after 2 days, open or done). No relations to the pet models. docs/GEO.md.
...
- **Prod does not run migrations**: `npm start` is `scripts/boot.js`, which runs `prisma db push --skip-generate` before `next start` (verified). Hence the constraint-avoidance patterns above and legacy columns kept (`Case.reporterToLastSeenMiles` — "live on prod, keep so db push stays additive").
...
**House rule (2026-07, hardened 2026-07-27 after the hat system was deleted)**: the top bar is IDENTICAL for every person on every route — same h-16 height, same Report CTA, same five center links (Pet Care · Lost & Found · Rescue Forces · Shelters · Hub), rendered from the frozen `CENTER_LINKS` array in `Navigation.js`. It does **not** vary with the route, with sign-in state, with shelter membership, or with anything else; the sole session-dependent slot is Sign in/Join vs bell + account menu. Anything person-specific (your shelter, your rescue forces) lives in the account menu, never in the bar. The short-lived Owner/Searcher "hat" chrome (`app/contexts/HatContext.js`, `docs/PRODUCT_IA_PLAN.md` §6) is **deleted**: a bar that rearranges itself as you move around reads as broken. `/care` redirects signed-in visitors to `/pets` (`app/care/CareGate.js`), which is how one Pet Care link serves guests and members without the bar changing. Two permitted variations only: **subtabs** below the bar (`sticky top-16`), and **whole-bar removal** inside immersive takeovers — each of which must ship a visible way back out (Mission Control's "Exit" → `/dashboard`, the portal's "Exit to ReunitePets" → `/`). **Probably Earth is a whole-bar removal, and it is the largest one.** The game at `/geo` used to be ordinary pages under this bar with its own subtabs below, and only a round or a room in progress took the screen. It got a name and an address of its own (probablyearth.com), and a player arriving there should not be looking at a navigation bar for a lost pet service, so the entire `/geo` tree is now a takeover on both hosts. It buys that the same way every other takeover does: a visible way out on every page. The game's own header carries its links, and `GeoFooter` carries "Made by ReunitePets" off the game entirely; the screens where even that header goes (a round, a room) keep their own X. `__tests__/global-chrome.test.js` checks both halves, the takeover and the way out.

Where chrome hides is decided in ONE place, `app/lib/navChrome.js` (`isImmersiveRoute` for the top bar — currently `/mission-control`, `/my-shelter` (the hat-gated shelter portal with its own sidebar chrome, `app/my-shelter/PortalShell.js`), `/rasuwa` (the Nepal flood letter tool: crisis page for a non-pet audience, way back out = the footer's ReunitePets link), and `/geo/play` + `/geo/room` + `/geo/script/play` (Probably Earth, the guessing game, while a round or a room is in progress; the HUD's X returns to the game's lobby, which, like the room browser and the rankings, is an ordinary page with the game's subtabs under the bar; docs/GEO.md). A build with `NEXT_PUBLIC_SITE=geo` (`app/lib/navChrome.js`, `SITE`) is the game's own site and has no pet chrome at all: one deployment is one site, decided at build time, never per host or per person; `hidesBottomNav` adds the report/join wizards and pet edit/medication wizards for the tab bar). Enforced by `__tests__/global-chrome.test.js`: no page/layout may ship its own `sticky top-0`/`fixed top-0` bar (page-local sub-headers anchor BELOW the bar with `sticky top-16 z-40` — see `/hub/search`, `/messages`), and the nav components may not grow ad-hoc route conditionals. Screens that must fill the viewport size themselves `h-[calc(100dvh-8rem)] lg:h-[calc(100dvh-4rem)]` (chat at `/messages/[id]`, `/simulate`, auth-page centering).
...
- **Second design system in the same file**: "Squad Hub — Bioluminescent" — `--hub-*` variables (near-black backgrounds, cyan/violet/yellow glows), `.squad-hub` + `.hub-aurora` + `.hub-grid`, urgency-pulsing card glows (`hub-case-high/medium/low`), dark Leaflet overrides, mission-control keyframes. Its only JS consumer, `components/squad/`, was deleted; the CSS remains for the mission-control surfaces.
- **Brand assets**: `frontend/lib/brandAssets.js` — all logos + Sarama mascot from `https://petrescue.b-cdn.net`. Icons: lucide-react.

### 8.5 Maps

Stack: `leaflet@1.9.4`, `react-leaflet@4.2.1`, `leaflet-draw@1.0.4`. All map components are client-only via `dynamic(..., { ssr: false })` with spinner placeholders. Tiles: every map draws OpenStreetMap tiles from `app/lib/maps/tiles.js` (dark screens invert them with `map-tiles-dark`; the light pages mute them with `map-tiles-calm`); satellite views use Esri World Imagery from the same module. `__tests__/map-tiles.test.js` fails on a tile address written anywhere else.

| Page | Map component |
|---|---|
| `/` | `BrowseMap` (dynamic) |
| `/lost-and-found` | `app/lost-and-found/BrowseMap.js` (list/map toggle) |
| `/cases/[caseNumber]` | `components/LastSeenMap.js` (leaflet, dynamic) |
| `/mission-control` | `MapCanvas` → `SARMapView` — full-bleed, always-mounted ("the map IS the mission") |
| `/rescue-forces/[id]` | `TerritoryMapCard` → `TerritoryMapInner` (light tiles) |
...
| `/geo/play` | `AppleLookAroundPane` + `AppleGuessMap` (MapKit JS Look Around). Apple is the only imagery: Google went in 2026-09 (founder direction), along with `GoogleStreetViewPane`, `GoogleGuessMap` and `app/geo/lib/googleMaps.js`. Not Earth (`NotEarthPane`, NASA panoramas) was retired 2026-09-23 |
| `/geo/script/play` | `AppleScriptMap` (MapKit, Muted Standard, `map.labels = false` and points of interest off, so Apple writes nothing on it; the country names are the game's own, from `country-labels.json`, and nothing smaller is ever named because in South Asia a state name is the answer). No imagery pane: the round is a sentence. The reveal draws the language's regions as polygon overlays, because the answer is an area. When MapKit says no (a clone, localhost, a preview, a spent quota) the round falls back to `LeafletScriptMap`, which draws the world from the bundled Natural Earth 1:110m polygons with no key, no quota and no bill; the refusal is latched for the rest of the game. Only an error moves the round: an earlier build read three seconds of silence as a refusal and put that fallback in front of players on a site where Apple works. Country names on both maps are placed by `app/geo/lib/countryLabels.js`, which drops a name whose box would land on one already drawn |
...
2. `/about-sarama`: framer-motion entrances — early screenshots catch opacity-0 elements.
3. `/simulator`: recharts renders only after a batch run completes.
4. `/rescue-forces/[id]`: the member tabs render only when signed in as a member.
...
**Required for local dev**: `DATABASE_URL` (`frontend/.env` / `.env.local`); `NEXTAUTH_SECRET`. The site's public address is built in, not configured: `getBaseUrl()` in `app/lib/config.js` returns `SITE_URL` from `app/lib/brand.js` everywhere except `next dev` (`http://localhost:3000`, or `NEXTAUTH_URL` for another port), and `auth.js` hands it to NextAuth. `NEXTAUTH_URL` and `NEXT_PUBLIC_BASE_URL` are ignored in production; enforced by `__tests__/site-address.test.js`.
...
- **`KNOWN_PRIVATE` (22, each with a reason comment)**: `pets/[id]` + `care`, `edit`, `medications`, `medications/new`, `share`, `health`, `today`; `messages/[id]`; `admin/missions/[missionId]`, `admin/users/[id]`, `admin/pets/[id]`; `missions/[missionNumber]/coordinate`; `rescue-forces/[id]/command-center`, `/chat`, `/divisions`, `/divisions/[divisionId]`, `/members`, `/mission-control`, `/settings`, `/updates`; `communities/[id]` (legacy, redirected).
