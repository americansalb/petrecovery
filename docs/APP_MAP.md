...
| `/geo/leaderboard` | Glicko ladders (classic, duel, ranked solo) with your own row, AND the rest of the game: Ranked, the daily challenge and the weekly cup with their boards (`Contests.js`), then country streak, one continent and one country (`OtherModes.js`). These were the deleted lobby's, and this is the page that inherited them | client | public | `GeoRating`, `GeoProfile` |
