...
**Monetization model** (decided 2026-06-10, `MONETIZATION.md` — supersedes older specs): Free forever — medication tracking incl. shared care teams (deliberately free where PetTimely charges $7.99/mo; it's the daily-habit funnel keeping recovery profiles accurate) and all of lost-pet recovery ("money only ever buys extra megaphone"). Paid: boosted reach — auto-built Facebook/Instagram ads from the pet's profile at a **flat $5 service fee + ad spend passed through at cost**, auto-paused when the pet is marked found. Future candidates (not committed): shelter management software ~$25/mo, clearly-labeled sponsorships. This replaced the earlier "December 2024"-stamped design (`MASTER_VISION.md`, `ADFUND_*`): 15% margin on crowdfunded ad spend ($5 min, $20 launch, LIFO refunds, 70/30 Meta/Google split, Stripe chargeback protection), reunion tips, and a $50–150/mo Shelter CRM projected to $20–65k/mo — fully specified but shelved.
...
**House rule (2026-07)**: the top bar is IDENTICAL on every route — same h-16 height, same links, same Report CTA — and hides only inside intentional immersive takeovers. Where chrome hides is decided in ONE place, `app/lib/navChrome.js` (`isImmersiveRoute` for the top bar — currently just `/mission-control`; `hidesBottomNav` adds the report/join wizards and pet edit/medication wizards for the tab bar). Enforced by `__tests__/global-chrome.test.js`: no page/layout may ship its own `sticky top-0`/`fixed top-0` bar (page-local sub-headers anchor BELOW the bar with `sticky top-16 z-40` — see `/hub/search`, `/messages`), and the nav components may not grow ad-hoc route conditionals. Screens that must fill the viewport size themselves `h-[calc(100dvh-8rem)] lg:h-[calc(100dvh-4rem)]` (chat at `/messages/[id]`, `/simulate`, auth-page centering).

- `app/components/Navigation.js` — THE universal sticky top bar (`sticky top-0 z-50 bg-midnight-900`, h-16). Renders on every route incl. auth pages; auth-STATE-adaptive only: members get My Pets + bell + user menu (admin links live inside the user menu, not the bar — the link set is identical for every role), guests get Sign in/Join; while the session resolves, a fixed-size placeholder holds the right slot so the bar never reflows. "Rescue Forces" is always the same dropdown trigger (user's squads from `/api/dashboard` only change its contents). Logo has an explicit 56px box (no CLS while the CDN image loads); labels are `whitespace-nowrap` with lg→xl compaction (icon-only logo, avatar-only chip, `px-3` pills below `xl`). Mobile: hamburger → 300px right drawer.
- `app/components/GlobalBottomNav.js` — mobile-only (`lg:hidden`) fixed bottom tab bar: Home(/dashboard), My Pets, raised circular **flash-yellow Report FAB** (`-mt-6` → `/report/new`), Lost & Found, Forces. Hidden per `hidesBottomNav` (immersive + wizard flows; shows on auth pages); includes `h-safe-area-inset-bottom` spacer.
...
**C. Route-local trees**: `app/cases/[caseNumber]/components/` (HeroSection, ActionCards, ActivityTimeline, MapPreview, SocialProofBar, StickyMobileCTA); `app/mission-control/components/` (MapCanvas, ActiveSearchScreen, ContextBar, MenuDrawer, MissionHeader; subdirs `desktop/`, `live/LiveSearchHUD`, `sheet/` (framer-motion BottomSheet), `simple/` (~17 panels), `overlays/` (`ConfettiBurst` pure-CSS confetti, `ReunitedCelebration`, `HelperBriefOverlay`), `regions/`, `modals/`); `app/simulator/components/`; `app/simulate/components/`.
