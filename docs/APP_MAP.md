...
|---|---|---|---|---|
| `/geo` | Lobby for the street-level guessing game (docs/GEO.md): provider (Google Street View / Apple Look Around beta), mode (world random, balanced, daily, continent, country, city streets, country streak), rules (rounds, timer, move/pan/zoom, probe radius), local stats/history | server layout (static share card) + client page | public | none (localStorage only) |
...
| `/geo/rooms` | Multiplayer room browser: open a room (classic or duel, places, rounds, timer, movement, public/private), join by code, public rooms active in the last 20 min | client | public | `GeoRoom` (list) |
...
| `geo/rooms/[code]/route.js` | GET, POST | Room state for the `x-geo-player` token (spectator without) / `join`, `start`, `guess`, `next`, `react`, `leave`, `rematch`. Every call ticks the room's clock; transitions are version-claimed | public | `GeoRoom*` |
