...
|---|---|---|---|---|
| `/geo` | Lobby for the street-level guessing game (docs/GEO.md): imagery (Apple Look Around by default, Google Street View), mode (world random, balanced, daily, weekly cup, continent, country, city streets, everywhere, kidnapped, country streak; on Apple every mode is a draw of city streets), rules (rounds, timer, format: Moving / No Move / NMPZ, probe radius), local stats/history | server layout (static share card) + client page | public | none (localStorage only) |
...
| `/geo/rooms` | Multiplayer room browser: open a room (classic or duel, imagery: Apple Look Around by default or Google Street View, places, rounds, timer, format, public/private), join by code, public rooms active in the last 20 min | client | public | `GeoRoom` (list) |
...
| `/geo/leaderboard` | Glicko ladders (classic, duel) with your own row | client | public | `GeoRating`, `GeoProfile` |
...
Chrome: the lobby, room browser, rankings and share pages are ordinary pages under the universal bar with the game's subtabs below it (`app/geo/components/GeoHeader.js`, `sticky top-16`: Play, Script, Rooms, Rankings, Daily, Profile); only `/geo/play`, `/geo/room/[code]` and `/geo/script/play` are immersive (`app/lib/navChrome.js`), each with an X back out. A build with `NEXT_PUBLIC_SITE=geo` is the game's own site (no pet chrome, the game's header, non-game paths redirect to the pet site); `GEO_DOMAINS` in `middleware.js` gives the same redirects to a host on the shared deployment (docs/GEO.md, "Hosting on another domain").
