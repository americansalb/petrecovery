...
- `app/components/Navigation.js` — universal sticky top bar (`sticky top-0 z-50 bg-midnight-900`). Auth-adaptive: desktop links My Pets, Pet Care, Lost & Found, Rescue Forces (dropdown of the user's squads from `/api/dashboard`), Hub, Admin dropdown (ADMIN); `NotificationBell` polls `/api/notifications?limit=1` every 60s; yellow "Report Pet" CTA (`bg-flash-400`) with Lost/Found split; guests get Sign in/Join. Mobile: hamburger → 300px right drawer with quick-action tiles. **Self-hides** on `/mission-control*` and auth pages.
- `app/components/GlobalBottomNav.js` — mobile-only (`lg:hidden`) fixed bottom tab bar: Home(/dashboard), My Pets, raised circular **flash-yellow Report FAB** (`-mt-6` → `/report/new`), Lost & Found, Forces. Hidden on mission-control, `/report/*`, `/join/*`, auth pages; includes `h-safe-area-inset-bottom` spacer.
