...
Chrome: the lobby, room browser, rankings and share pages are ordinary pages under the universal bar with the game's subtabs below it (`app/geo/components/GeoHeader.js`, `sticky top-16`); only `/geo/play` and `/geo/room/[code]` are immersive (`app/lib/navChrome.js`), each with an X back out. A build with `NEXT_PUBLIC_SITE=geo` is the game's own site (no pet chrome, the game's header, non-game paths redirect to the pet site); `GEO_DOMAINS` in `middleware.js` gives the same redirects to a host on the shared deployment (docs/GEO.md, "Hosting on another domain").
